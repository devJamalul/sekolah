              ...
              <form action="{{ route('transactions.update', $student->getKey()) }}" method="post">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control select2 @error('payment_type_id') is-invalid @enderror"
                    ...
                    @foreach ($payment_types as $payment_type)
                      <option value="{{ $payment_type->getKey() }}" @selected(old('payment_type_id') == $payment_type->getKey())>{{ $payment_type->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control @error('nominal') is-invalid @enderror" name="nominal"
                    id="nominal" aria-describedby="nominal" value="{{ old('nominal') }}" autocomplete="off">
                  @error('nominal')
                    <div class="invalid-feedback">
                      {{ $message }}
                    </div>
